...
parser.add_argument('--margin_1', type=float, default=0.3,
...
parser.add_argument('--loss_function', default='triplet', choices=[
                    'triplet'], help='triplet or quadruplet [default: quadruplet]')
...
cfg.TEST_FILE = '/mnt/generating_queries/bev-ox/test_queries_baseline.pickle'
...
def train_one_epoch(model, optimizer, train_writer, loss_function, epoch):
    ...
    for i in range(len(train_file_idxs)//cfg.BATCH_NUM_QUERIES):
            ...
            if (len(TRAINING_LATENT_VECTORS) == 0):
                q_tuples.append(
                    get_query_tuple(TRAINING_QUERIES[batch_keys[j]], cfg.TRAIN_POSITIVES_PER_QUERY, cfg.TRAIN_NEGATIVES_PER_QUERY,
                                    TRAINING_QUERIES, hard_neg=[], other_neg=False ))
                ...
                q_tuples.append(
                    get_query_tuple(TRAINING_QUERIES[batch_keys[j]], cfg.TRAIN_POSITIVES_PER_QUERY, cfg.TRAIN_NEGATIVES_PER_QUERY,
                                    TRAINING_QUERIES, hard_negs, other_neg=False))
        ...
        for j in range(25):  
            ...
            # Query 
                final_image_list[j].append(q_tuples[k][0][0][0][j])
                final_point_list[j].append(q_tuples[k][0][0][1][j])

            # Pos   
                final_image_list[j].append(q_tuples[k][1][0][0][j])
                final_point_list[j].append(q_tuples[k][1][0][1][j])

            # Neg 
                for neg_index in range(len(q_tuples[k][2])):  
                    final_image_list[j].append(q_tuples[k][2][neg_index][0][j])
                    final_point_list[j].append(q_tuples[k][2][neg_index][1][j])
        ...
        # output_queries, output_positives, output_negatives= run_model(model, queries_img_tensor ,positives_img_tensor,negatives_img_tensor,queries_pts_tensor,positives_pts_tensor,negatives_pts_tensor)
        output_queries, output_positives, output_negatives= run_model(model,final_structure)
        loss = loss_function(output_queries, output_positives, output_negatives,output_queries ,0.3,0.3, use_min=cfg.TRIPLET_USE_BEST_POSITIVES, lazy=True, ignore_zero_loss=cfg.LOSS_IGNORE_ZERO_BATCH)
        loss.backward()
        optimizer.step()
...
def run_model(model, final_structure , require_grad=True):


    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    

    image_tensors = [tensor.to(device).requires_grad_(require_grad) for tensor in final_structure[0]]
    point_tensors = [tensor.to(device).requires_grad_(require_grad) for tensor in final_structure[1]]
    

    if require_grad:
        output = model([image_tensors, point_tensors])
    else:
        with torch.no_grad():
            output = model([image_tensors, point_tensors])
    ...
    o1, o2, o3 = torch.split(
        output, [1, cfg.TRAIN_POSITIVES_PER_QUERY, cfg.TRAIN_NEGATIVES_PER_QUERY], dim=1)

...
def construct_query_and_database_sets(base_path, runs_folder, folders, pointcloud_fols, filename, p, output_name):
    ...
    for folder in folders:
        ...
        for index, row in df_locations.iterrows():
            ...
            if output_name == "business" or check_in_test_set(row['northing'], row['easting'], p, x_width, y_width):
                test_rows.append(row)
            database_rows.append(row)

        df_database = pd.DataFrame(database_rows, columns=['file', 'northing', 'easting'])
        df_test = pd.DataFrame(test_rows, columns=['file', 'northing', 'easting'])
    ...
    for folder in folders:
        ...
        df_locations['timestamp'] = runs_folder+folder + \
            pointcloud_fols+df_locations['timestamp'].astype(str)+'.png'
...
base_path = "/data/soomoklee/data/bev_benchmark_datasets"
